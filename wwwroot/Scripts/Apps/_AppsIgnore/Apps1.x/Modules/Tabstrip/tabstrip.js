...
define(['./modules/util.js'], function (Util) {

    var Me = {

        BaseFolder: '../../Scripts/Tabstrip',

        Initialize: function (tabstripId, serverMode, baseFolder) {
            ...
            Me.CreateTabstrips(tabstripId, serverMode);

        },

        CreateTabstrips: function (tabstripId, serverMode)
        {
                ...
                if (ts.id === tabstripId) {
                    ...
                    //if (newTabstripHTML.length === 0)
                    {
                        ...
                        $.each($(ts).children(), function (tabIndex, tab) {
                            ...
                            newTabstripTabsHTML += '<label class="tabstrip-label" onclick="Tabstrips.Select(\'' + tabstripId.trim() + '\', ' + tabIndex + ', ' + serverMode + ');" for="css3-tabstrip-0-' + tabIndex + '">' + tabTitle;
        ...
        Select: function(tabstripId, tabIndex, serverMode)
        {
            ...
            else
            {
                ...
                //alert(myPageName);
                location.href = 'Statistics.aspx?tab=' + tabIndex;
            }
        },
