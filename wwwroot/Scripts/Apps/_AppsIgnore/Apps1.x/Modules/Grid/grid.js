...
define(['../util.js'], function (Util)
{
    var Me = {
        ...
        RowActionCallback: function (callback, td) {
            ...
            var rowdata = unescape($($(td).parent().parent().parent().parent().parent()[0]).attr("rowdata"));
        ...
        RefreshHTML: function (settings) {
            ...
            if (settings.columns) {
                ...
                if (settings.title) {
                    ...
                    rows += '<tr>';
                    rows += '<td colspan="' + columns.length + '" style="border-top:0px;"><h2>' + title;
                    if (settings.add)
                ...
                if(settings.rowactions)
                    rows += '<td style="width:1%;border-top:0px;"></td>'; //holds actions
                ...
                rowbuttonheader = '<td style="width:' + rowbuttonwidthpercent + '%;border-top:0px;"></td>'; //holds buttons
                ...
                if (settings.rowbuttons) {
                    ...
                    $.each(settings.rowbuttons, function (index, button) {
                            ...
                            rowbuttons += '<input type="button" class="btn btn-xs btn-primary" value="' + button.text + '" onclick="Apps.Grids.RowButtonCallback(' + button.buttonclick.toString() + ',this);" /> ';
                    });
                ...
                $.each(columns, function (index, col) {
                    ...
                    if (col.hidden === undefined || col.hidden === false) { //Check if hidden
                        ...
                        rows += '<td style="width:' + col.widthpercent + '%;border-top:0px;" ' + columnclick + '><h4>' + col.text + '</h4></td>';
                    }
                });
                ...
                $.each(data, function (index, d) {
                    ...
                    if (settings.rowactions) {
                        ...
                        rows += '<td>';
                        rows += '<div class="btn-group"> ';
                        rows += '   <button type="button" class="btn btn-xs btn-primary dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"> ';
                        rows += '       Actions <span class="caret"></span> ';
                        rows += '   </button> ';
                        rows += '  <ul class="dropdown-menu"> ';
                        ...
                        if (cols.length === 1) {
                            ...
                            if (col.hidden === undefined || col.hidden === false) {
                                ...
                                //EDIT
                                    ...
                                    rows += '<td class="' + settings.id + 'Cell editCellStyle" index="' + index + '">';
                                    rows += viewSpan;
                                    rows += editSpan;
                                    rows += '</td>';
